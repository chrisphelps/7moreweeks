...
--import Signal exposing (Signal, (<~), (~), foldp, map, sampleOn)  can't use <~ and ~ anymore
...
input = sampleOn delta (map3 Input 
    Keyboard.space
    Mouse.x
    delta)
...
stepGamePlay {space, x, delta} ({state, heads, player, seed} as game) =
    let (rand, seed') = generate (int 0 4) seed
    in 
        { game | 
...
allHeadsSafe x heads = all (headSafe x) heads
...
spawnHead score heads rand = 
    let
        addHead = length heads < (score // 5000 + 1) && all (\head -> head.x > 107.0) heads
    in
        if addHead then defaultHead rand :: heads else heads

bounceHeads heads = List.map bounce heads

bounce head = { head | vy = if head.y > bottom && head.vy > 0 then
        -head.vy * 0.95
    else
        head.vy }
...
drawBuilding w h =
    filled red (rect 100 (toFloat h)) |> moveX (-(half w) + 50)
...
drawMessage w h state =
    toForm (txt (height 50) (stateMessage state)) |> move (50, 50)
